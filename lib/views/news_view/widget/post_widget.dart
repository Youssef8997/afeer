        ...
        children: [
          Text(
            ...
            style: FontsManger.mediumFont(context)
                ?.copyWith(fontSize: 15, color: const Color(0xff1D1F23)),
          ),
          Text(
            "${widget.post.time!.toDate().month}/${widget.post.time!.toDate().day}",
            style: FontsManger.mediumFont(context)
                ?.copyWith(fontSize: 13, color: const Color(0xff606770)),
          ),
          const SizedBox(
            height: 10,
          ),
          Text(
            widget.post.title,
            style: FontsManger.mediumFont(context)
                ?.copyWith(color: const Color(0xff1D1F23), fontSize: 16),
          ...
          if (widget.post.linkPdf != null)
            IconButton(
                onPressed: () => navigatorWid(
                    page: PdfView(pdfLink: widget.post.linkPdf!),
                    context: context,
                    returnPage: true),
          ...
          Row(
            children: [
              Text(
                "تعليق ${widget.post.comments?.length}",
                style: FontsManger.mediumFont(context)
                    ?.copyWith(color: const Color(0xff606770), fontSize: 15),
              ),
              const Spacer(),
              Text.rich(TextSpan(children: [
                TextSpan(
                  text: widget.post.likedId?.length.toString(),
                  style: FontsManger.mediumFont(context)
                      ?.copyWith(color: const Color(0xff606770), fontSize: 15),
                ),
                WidgetSpan(
                                        ...
                                        onPressed: () {
                                          if (context.appCuibt.isVisitor ==
                                            ...
                                            CommentModel comment = CommentModel(
                                                idUser: context
                                                    .appCuibt.user!.token,
                                                idComment: const Uuid().v4(),
                                                comment: commentController.text,
                                                time: Timestamp.now());
                                            context.appCuibt.addComment(
                                                comment, widget.post);
                                            setState(() {
                  ...
                  children: [
                    Text(
                      ...
                      style: FontsManger.mediumFont(context)?.copyWith(
                          fontSize: 10, color: const Color(0xff606770)),
                    ),
                    const SizedBox(
                      width: 5,
                    ),
                    const Icon(Icons.chat_bubble_outline,
                        size: 20, color: Color(0xff606770)),
                  ],
                ),
              ),
              InkWell(
                ...
                child: Row(
                  children: [
                    Text(
                      ...
                      style: FontsManger.mediumFont(context)?.copyWith(
                          fontSize: 10,
                          color: widget.post.likedId!
                                  .contains(context.appCuibt.user?.token)
                              ? Colors.blue
                              : const Color(0xff606770)),
                    ),
                    const SizedBox(
                      width: 5,
                    ),
