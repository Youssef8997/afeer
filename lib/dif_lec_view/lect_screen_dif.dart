...
import 'diff_lec_screen.dart';


class LectureDiffScreens extends StatefulWidget {
  final String subjectName;
  final String collection;
  const LectureDiffScreens({super.key, required this.subjectName, required this.collection,});

  @override
  State<LectureDiffScreens> createState() => _LectureDiffScreensState();
}

class _LectureDiffScreensState extends State<LectureDiffScreens> {
  @override
                ...
                if(context.appCuibt.lectureAdditionalList.isNotEmpty)
                  SizedBox(
                      ...
                      child: ListView.separated(itemBuilder: (context,i)=>ListTile(
                        onTap: ()=>navigatorWid(page: LectureDetDiffScreen(subjectName: widget.subjectName,lecture:context.appCuibt.lectureAdditionalList[i],collection: widget.collection, ),context: context,returnPage: true),
                        ...
                        title: Text(context.appCuibt.lectureAdditionalList[i].name,style:FontsManger.largeFont(context) ),
                      ), separatorBuilder: (context,i)=>const Divider(color: Color(0xffDFDFDF),thickness: 1,), itemCount: context.appCuibt.lectureAdditionalList.length)),
